    ...
    if (styleSheet.href.endsWith('article.css')) {
      for (const rule of styleSheet.cssRules) {
        ...
        if (selectorText === '#article-text') {
          ...
          rule.style.width = paper.width;
          rule.style.height = paper.height;
        } else if (!selectorText && cssText.startsWith('@page')) {
          rule.style.size = paper.size;
        }
      }
    }
  }
...
export function getPageProperties() {
  const settings = getSettings();
  const paper = getPaperProperties(settings.paper);
  const { width, height, defaultMargins } = paper;
  const margins = (settings.margins instanceof Object) ? settings.margins : defaultMargins;
  return { width, height, margins, footerGap: footerMarginTop };
}
